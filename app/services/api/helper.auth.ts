...
import {API_URL, getAuthUrl} from "@/config/api.confige";
...
export const getNewTokens = async  () => {
    try {
        ...
        const response = await axios.post<string, {data: IAuthResponse}>(API_URL + getAuthUrl
        ('/login/access-token'), {refreshToken}, {
            headers: {
